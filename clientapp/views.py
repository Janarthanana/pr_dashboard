...
from .models import TicketRegister, TicketCreation
...
def index(response):
    ...
    context = {'count': count}
    # l=TicketCreation.objects.filter(assignee=request.user).count()
...
class TicketInsert(View):
    def get(self, request):
        ...
        print(obj)
        userss = {'id': obj.id, 'organization': organization,
                  'subject': subject, 'message': message, 'prioritylevel': prioritylevel}
...
class TicketView(ListView):
    ...
    # print(TicketCreation.objects.filter(assignee=request.user.username).count())
    # print(TicketCreation.objects.filter(assignee='hggh').count())

# class MyTicket(ListView):
# 	def tfunc(self):
# 		t=self.request.user
# 		print(t)
...
class TicketInsertion(View):
    def get(self, request):
        ...
        # creator1 = request.GET.get('creator', None)
        # TicketInsertion.User.objects.get()
